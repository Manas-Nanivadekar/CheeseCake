...
const AgendaSection = () => {
  ...
  return (
    <section className="w-full bg-[#FFFC6D] min-h-screen">
        ...
        <div className="grid gap-12 lg:grid-cols-2 items-center">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              <Button className="bg-[#0A0B1F] text-white hover:bg-[#3EEEC0] transition-colors duration-300 px-8 py-6 text-lg rounded-full">
                Get started
              </Button>
              <Button
                variant="outline"
          ...
          <div className="relative">
            <div className="bg-[#94a4ff] rounded-2xl p-4 shadow-xl">
              <img
                src="https://images.www.talentlms.com/wp-content/uploads/2018/08/what-is-lms-1-1024x273.jpg"
      ...
      <div className="min-h-screen bg-[#f7f8fa] py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto space-y-12">
            {/* Upload Section */}
            <Card
              className={`p-6 ${currentStep !== "upload" ? "opacity-50" : ""}`}
            >
              <h2 className="text-2xl font-bold text-[#0A0B1F] mb-6">
                Create New Pathway
              </h2>
            ...
            {/* Pathway Name */}
            <Card
              className={`p-6 ${currentStep !== "name" ? "opacity-50" : ""}`}
            >
              <Label
                htmlFor="pathway-name"
                className="text-lg font-medium text-[#0A0B1F] mb-2 block"
              >
                Pathway Name
              </Label>
              <div className="flex gap-4">
            ...
            <div
              className={`space-y-6 ${
                currentStep !== "templates" ? "opacity-50" : ""
              }`}
            >
            ...
            {currentStep === "build" && (
              <div className="flex justify-center pt-8">
                <Button
                  onClick={handleStart}
                  className="bg-[#0A0B1F] text-white hover:bg-[#3EEEC0] transition-colors duration-300 px-8 py-6 text-lg rounded-full relative"
                  disabled={loading}
                >
                  {loading ? (
      ...
      {/* Cheesie Guide */}
      <CheesieGuide currentStep={currentStep} />
    </section>
  );
};
