...
const defaultQuizData = {
    ...
    type: "website",
    context: "Welcome to the LISA AI learning experience! LISA is an innovative platform that combines artificial intelligence with learning management to create personalized, engaging, and effective learning experiences.",
    questions: {
        mcq: {
            question: "What feature of LISA AI helps ensure regulatory adherence through regular training and certifications?",
            options: {
            ...
            correct_answer: "A",
            explanation: "Compliance Assessments are specifically designed to ensure that employees receive regular training and certifications necessary for regulatory adherence. This feature facilitates ongoing education and certification in compliance-related areas, helping organizations stay up-to-date with regulations and avoid potential legal issues."
        },
        true_false: {
            questions: [
                {
                    statement: "The integration capabilities of LISA AI are designed to only work with specific, pre-approved tools, limiting its adaptability.",
                    answer: false,
                    explanation: "This statement is false because LISA's integration capabilities are portrayed as being designed to effortlessly connect with a variety of tools according to the provided context, indicating adaptability and a broad range of compatibility rather than limitation to specific tools."
                },
                {
                    statement: "Certifications awarded for completed courses can serve to enhance both credibility and motivation among learners.",
                    answer: true,
                    explanation: "This statement is true as the context directly mentions that certifications are awarded for completed courses, enhancing credibility and motivation. This suggests that certifications act as a tangible acknowledgment of achievement, which can validate a learner's effort and knowledge, thereby motivating them and others."
                }
            ]
        },
        fill_blanks: {
            question: "LISA's integration capabilities are designed to effortlessly connect with a variety of tools, ensuring _____ and _____ connections.",
            answers: [
                "Seamless",
                "Powerful"
            ],
        ...
        ranking: {
            scenario: "Your organization has recently decided to overhaul its Learning and Development (L&D) strategy to address the rapid pace of technological advancements and the diverse needs of its global workforce.",
            question: "What is the correct order of steps to successfully implement LISA in your organization?",
            options: [
            ...
            explanation: "The correct order begins with integrating LISA with existing tools and systems (Step 3) to ensure that all organizational knowledge and resources are accessible through the platform."
        }
    }
}
...
const GamefiedQuiz: React.FC = () => {
    ...
    const handleTrueFalseAnswer = (answer: boolean) => {
        const currentQuestion = defaultQuizData.questions.true_false.questions[currentTrueFalseIndex]
        ...
        if (currentTrueFalseIndex < defaultQuizData.questions.true_false.questions.length - 1) {
            setTimeout(() => {
            ...
            case 0:
                return (
                    <Card className="w-full max-w-2xl">
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold text-primary">Welcome to the LISA AI Quiz!</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Cheesie
                                emotion="excited"
            ...
            case 1:
                return (
                    <MultipleChoiceQuestion
                        ...
                        options={Object.entries(defaultQuizData.questions.mcq.options).map(([id, text]) => ({ id, text }))}
                        correctAnswer={defaultQuizData.questions.mcq.correct_answer}
                        onAnswer={handleAnswer}
                    />
                )
            case 2:
                const currentTFQuestion = defaultQuizData.questions.true_false.questions[currentTrueFalseIndex]
                return (
                    <Card className="w-full max-w-2xl">
                        <CardHeader>
                            <CardTitle className="text-2xl font-bold text-primary">True or False</CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Cheesie
                                emotion="thinking"
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex space-x-2 mt-2">
                                    <Button onClick={() => handleTrueFalseAnswer(true)}>True</Button>
                                    <Button onClick={() => handleTrueFalseAnswer(false)}>False</Button>
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="w-full max-w-2xl">
                        ...
                        <CardContent>
                            ...
                            <div className="mt-4 text-center">
                                ...
                                <span className="text-2xl font-bold">Your Score: {score} points</span>
                            </div>
                            <Button 
                                onClick={() => {
                                    setCurrentStep(0)
    ...
    const getCurrentExplanation = () => {
        switch (currentStep) {
            ...
            case 2:
                return defaultQuizData.questions.true_false.questions[currentTrueFalseIndex].explanation
            case 3:
    ...
    return (
        <div className="container mx-auto p-4 min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-blue-100 to-purple-100">
            <h1 className="text-4xl font-bold mb-8 text-center text-primary">LISA AI Learning Adventure</h1>
            ...
            {showExplanation && (
                <Card className="w-full max-w-2xl mt-4">
                    <CardContent>
                        <Cheesie
                            emotion={isCorrect ? "excited" : "thinking"}
                            message={isCorrect ? "Great job! Here's why:" : "Not quite. Here's the explanation:"}
                        />
                        <p className="mt-2">{getCurrentExplanation()}</p>
                        {(currentStep !== 2 || currentTrueFalseIndex === defaultQuizData.questions.true_false.questions.length - 1) && (
