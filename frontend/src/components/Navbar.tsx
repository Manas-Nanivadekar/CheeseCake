...
const navItems = [
  {
    ...
    dropdownItems: [
      ...
      { title: "Explore Pathway", href: "/pathway/view" },
    ],
  },
  {
    title: "Leaderboard",
    ...
    >
      <ul className="grid gap-6 px-4">
        {items.map((item) => (
          ...
          >
            {item.dropdownItems ? (
              ...
              <Link
                to={item.href}
                className={`text-[#0A0B1F] hover:opacity-70 transition-opacity duration-300 ${
                  pathname === item.href ? "text-[#3EEEC0]" : ""
                }`}
                onClick={() => setIsOpen(false)}
              >
                {item.title}
        ...
        <motion.li whileHover={{ scale: 1.02 }}>
          {!loggedIn && <button className="sign-up-button">Sign Up</button>}F
        </motion.li>
      </ul>
    </motion.nav>
  );
    ...
    >
      <div className="container flex h-16 max-w-screen-2xl items-center justify-between px-8">
        <motion.div className="flex items-center" whileHover={{ scale: 1.05 }}>
          <Link
            to="/"
            className="text-[#0A0B1F] flex text-xl font-bold transition-colors gap-2 items-center justify-center duration-300"
          >
            <img
              src="https://res.cloudinary.com/dnvh2fya6/image/upload/v1736544667/cheese_aen61j.png"
              alt=""
              height={20}
              width={30}
            />
            ...
            <Button
              ...
              onClick={() =>
                (window.location.href =
                  "https://www.github.com/Manas-Nanivadekar/CheeseCake")
              }
            >
              Know more
            </Button>
